...
ap.add_argument("-i", "--image", required=True,
	help="path to input image")  #在参数中去指定输入图像的路径
ap.add_argument("-t", "--template", required=True,
	help="path to template OCR-A image") #在参数中去指定模板的路径
...
ref_, refCnts, hierarchy = cv2.findContours(ref.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE) #注意传进去的图像要进行一个.copy（）,返回的参数refCnts是轮廓
...
for (i, (gX, gY, gW, gH)) in enumerate(locs):
	...
	for c in digitCnts:
		...
		roi = cv2.resize(roi, (57, 88)) #要做模板匹配，故大小要一样的
		cv_show('roi',roi)
